//=========================================================
//                  METROPOLIS CON GAUSS TRANSITION
//=========================================================
...
// Inizializzazione generatore numeri casuali
   ...
   */
   ...
   double S = 0.872; // modulo del passo del random walk
   ...
   process = "Equilib_GS";
   std::vector<vector<double>> Equilibration_GS = rnd.Metropolis_sampling_gauss (p_GroundState, S, M, extractions, process); // eseguo il random walk
  ...
  L = extractions / N;
  process = "GS";
  std::vector<vector<double>> GS = rnd.Metropolis_sampling_gauss (p_GroundState, S, M, extractions, process); // eseguo il random walk
   //Trasformo le posizioni in distanze
  ...
  S = 1.42; // modulo del passo del random walk
  ...
  process = "Equilib_2P";
  std::vector<vector<double>> Equilibration_2P = rnd.Metropolis_sampling_gauss (p_2Pstate, S, M, extractions, process); 
  //Trasformo le posizioni in distanze
 ...
 L = extractions / N;
 process = "2P";
 std::vector<vector<double>> State_2P = rnd.Metropolis_sampling_gauss (p_2Pstate, S, M, extractions, process); // eseguo il random walk
  //Trasformo le posizioni in distanze
...
//=================================================================
//-------------------STAMPA DEI PUNTI ESTRATTI DALLA PDF-----------
//=================================================================

//Apro il file in cui voglio scrivere i risultati
   cout<<"Procedo con scrittura dei punti estratti per lo stato 2P"<<endl;
   std::ofstream file("Punti_estratti_2P.txt");
   if (!file.is_open()) {
       cerr << "Error: unable to open Punti_estratti_2P.txt" << endl;
       exit(1);
     }
   file << setw(15) << "x" << setw(15) << "y" << setw(15) << "z" << "\n";

   extractions = 2E5;  //numero di punti da estrarre, dopo equilibrazione
   std::vector<vector<double>> Points_2P = rnd.Metropolis_sampling_gauss (p_2Pstate, S, M, extractions, process); // eseguo il random walk

   for (int i=0; i<extractions; i++) {
      file << setw(15) << Points_2P[i][0] << setw(15) << Points_2P[i][1] << setw(15) << Points_2P[i][2] << "\n";
   }
   //Chiudo il file
   file.close();
   cout<<"Processo terminato"<<endl;
