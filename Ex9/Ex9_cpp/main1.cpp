//=========================================================
// ALGORITMO GENETICO APPLICATO A TRAVELING SALESMAN PROBLEM
//=========================================================
...
vector<int> selector (vector<vector<int>> & Population, int M, Random & rnd) {
   ...
   int index_chosen = static_cast<int>(M*pow(rnd.Rannyu(), 2));   
...
void Genetic (int M, int G, vector<vector<double>> cities, int generations, Random & rnd, double prob_mut, double prob_cross, string type_path) {
...
// Inizializzazione generatore numeri casuali
   ...
   int M = 150; // numero di individui nella popolazione
   int G = 34; //numero di geni per individuo (aka numero di città da visitare)
   int generations = 1000 ; //numero di generazioni, ovvero numero di volte che viene creata una nuova popolazione a partire da quella precedente
   ...
   //ALGORITMO GENETICO 
   double prob_mut = 0.08;
   double prob_cross = 0.6;
   ...
   Genetic (M, G, cities_squar, generations, rnd, prob_mut, prob_cross, type_path);
   ...
   //Per cerchio
   type_path = "Circle";
   Genetic (M, G, cities_circ, generations, rnd, prob_mut, prob_cross, type_path);
