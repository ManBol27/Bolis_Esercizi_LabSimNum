...
/**************************************************************************************/
/********************FUNZIONE ALGORITMO METROPOLIS - Transizione Normale  *************/
/**************************************************************************************/
...
/**************************************************************************************/
/********************FUNZIONE ALGORITMO METROPOLIS - Transizione Uniforme *************/
/**************************************************************************************/

//Funzione per fare un sampling di una qualsiasi funzione p(x,y,z) con algoritmo metropolis
//devo passare la funzione p(x,y,z) come argomento
//ritorna una posizione x,y,z
std::vector<std::vector<double>>Random :: Metropolis_sampling (double (*p)(double, double, double, double, double), double S, int M, int extractions, std::string process, double mu, double sigma){
  ...
  for (int i=0; i<M; i++){ //ciclo per M passi
    ...
    //scrivo la funzione di accettazione, ipotesi T(x,y) = T(y,x) (simmetria della transizione proposta)
    double A = std :: min(1., p(X_position[0]+passo[0],X_position[1]+passo[1],X_position[2]+passo[2], mu, sigma)/p(X_position[0],X_position[1],X_position[2], mu, sigma));
    double r = Rannyu();
    if (r < A) { //accetto il passo
      X_position[0] += passo[0];
      X_position[1] += passo[1];
  ...
  for (int i=1; i<extractions; i++){ //parto dall'indice i = 1 perchè la posizione 0 è gia settata nelle righe sopra
    ...
    passo [2] = S * Rannyu(-1.,1.);  
    //scrivo la funzione di accettazione, ipotesi T(x,y) = T(y,x) (simmetria della transizione proposta)
    double A = std :: min(1., p(X_sampled[i-1][0]+passo[0],X_sampled[i-1][1]+passo[1],X_sampled[i-1][2]+passo[2], mu, sigma)/p(X_sampled[i-1][0],X_sampled[i-1][1],X_sampled[i-1][2], mu, sigma));
    double r = Rannyu();
    if (r < A) { //accetto il passo
      X_sampled[i][0] = X_sampled[i-1][0] + passo[0];
      X_sampled[i][1] = X_sampled[i-1][1] + passo[1];
  ...
  double acc_rate = double(accepted)/extractions;

  if (acc_rate < 0.485 or acc_rate > 0.515) {
    return {}; //ritorno un vettore vuoto se l'acceptance rate non è compreso tra 0.485 e 0.515 così da dare un messaggio di errore
  }
  else return X_sampled; //ritorno il vettore con le estrazioni in 3d
}
