...
// Inizializzazione generatore numeri casuali
  ...
  andrò a generare un numero M di numeri casuali raggruppati in N blocchi  */
   ...
   for(int i = 0; i < M; ++i) {
      r[i] = static_cast<int>(rnd.Rannyu(0,6));       // genero M numeri interi casuali estratti uniformemente tra 0 e 5
   }
   ...
   for(int j = 0; j < L; ++j) {
      ...
      for(int i=0; i<N; ++i) {
         int k = i + j * N;
         if (r[k] == 0) x_pos += 1;
         else if (r[k] == 1) x_pos -= 1;
         else if (r[k] == 2) y_pos += 1;
         else if (r[k] == 3) y_pos -= 1;
         else if (r[k] == 4) z_pos += 1;
         else if (r[k] == 5) z_pos -= 1;
   ...
   string File_name = "RW_discreto.txt";
