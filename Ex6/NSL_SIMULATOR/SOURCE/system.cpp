...
void System :: read_configuration(){
  ...
  if(_restart){
    cinf.open("../INPUT/CONFIG/config_termalized.xyz");
    if(cinf.is_open()){
      string comment;
      string particle;
      double x, y, z;
      ...
      cinf >> ncoord;
      if (ncoord != _npart){
        cerr << "PROBLEM: conflicting number of coordinates in input.dat & config_termalized.xyz not match!" << endl;
        exit(EXIT_FAILURE);
      }
      cinf >> comment;
      for(int i=0; i<_npart; i++){
    ...
    } else cerr << "PROBLEM: Unable to open INPUT file config_termalized.xyz"<< endl;
