...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    if(this->gibbs(i)) {
      _particle(i).setspin(+1);     //se gibbs propone spin up, metto quella particella spin up
    }
...
bool System :: gibbs(int i){ // Gibbs algorithm
  bool decision = false;
  double delta_E, acceptance;
  if(_sim_type == 1) delta_E = this->Boltzmann(i,true) - this->Boltzmann(i,false);
  else delta_E = 2.0 * (_J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() ) + _H) ;
  acceptance = 1./ (1 + exp(-_beta*delta_E));     // coincide con la probabilità che la particella i-esima abbia spin up
  if(_rnd.Rannyu() < acceptance ) decision = true; //Gibbs acceptance step
  return decision;
}
...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = 8./3. * M_PI * _rho * _npart * (1./3. * pow((1./_r_cut),9) - pow((1./_r_cut),3)); // TO BE FIXED IN EXERCISE 7
        ...
        index_property++;
        _ptail = 32./3. * M_PI * pow(_rho, 2) * (1./3. * pow((1./_r_cut),9) - 1./2. * pow((1./_r_cut),3)) ; // TO BE FIXED IN EXERCISE 7
  ...
  _nattempts = 0;
  _naccepted = 0;
  cout<<"Pressure index: "<<_index_pressure<<endl;
  cout<<"U/N index: "<<_index_penergy<<endl;
  cout<<"gofr index: "<<_index_gofr<<endl;
  cout<<"Last gofr index: "<<_index_gofr+_n_bins-1<<endl;
  return;
}



void System :: initialize_properties_T_(){ // Inizializza i files che contengono le grandezze misurate al variare di T
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "TOTAL_ENERGY" ){
        // Aggiungo l'apertura di un file che conterrà energia totale ultimo blocco in funzione della temperatura
        ofstream coutt1("../OUTPUT/tot_energy(T).dat");
        coutt1 << "#     Temperature:    Tot_energy:         ERROR:" << endl;
        coutt1.close();
        _nprop++;
      } else if( property == "MAGNETIZATION" ){
        // Aggiungo l'apertura di un file che conterrà energia totale ultimo blocco in funzione della temperatura
        ofstream coutpr1("../OUTPUT/magnetization(T).dat");
        coutpr1 << "#     Temperature:    Magnetization:         ERROR:" << endl;
        coutpr1.close();
      } else if( property == "SPECIFIC_HEAT" ){
        // Aggiungo l'apertura di un file che conterrà energia totale ultimo blocco in funzione della temperatura
        ofstream coutpr1("../OUTPUT/Specific_Heat(T).dat");
        coutpr1 << "#     Temperature:    Spec_Heat:         ERROR:" << endl;
        coutpr1.close();
      } else if( property == "SUSCEPTIBILITY" ){
        // Aggiungo l'apertura di un file che conterrà energia totale ultimo blocco in funzione della temperatura
        ofstream coutpr1("../OUTPUT/Susceptibility(T).dat");
        coutpr1 << "#     Temperature:    Susceptibility:         ERROR:" << endl;
        coutpr1.close();
      } else if( property == "ENDPROPERTIES" ){
        break;
      } else cerr << "PROBLEM: unknown property" << endl;
...
void System :: read_configuration(){
  ...
  if(_restart){
    cinf.open("../INPUT/CONFIG/config_equilibrated.xyz");
    if(cinf.is_open()){
      string comment;
      string particle;
      double x, y, z;
      ...
      cinf >> ncoord;
      if (ncoord != _npart){
        cerr << "PROBLEM: conflicting number of coordinates in input.dat & config_equilibrated.xyz not match!" << endl;
        exit(EXIT_FAILURE);
      }
      cinf >> comment;
      for(int i=0; i<_npart; i++){
    ...
    } else cerr << "PROBLEM: Unable to open INPUT file config_equilibrated.xyz"<< endl;
...
void System :: measure(){ // Measure properties
  ...
  int bin = 0;
  std::vector<double> gofr_temp(_n_bins); //temporary accumulator for g(r) values
  double virial=0.0;
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(dr < _halfside[0]) {
          bin = static_cast<int>(dr/_halfside[0] * _n_bins);  // restituisce quale è il bin in cui compare la misurazione appena effettuata
          _measurement(_index_gofr + bin) += 2;   //metto le due particelle nel bin 
        }
        if(dr < _r_cut){
    ...
    // Inserisco qui la normalizzazione per GOFR
    for(int i=0; i<_n_bins; i++) {
      _measurement(_index_gofr + i) /= (_rho * _npart * 4./3. * M_PI * (pow((double(i)/double(_n_bins) * _halfside[0] + _bin_size),3) - pow((double(i)/double(_n_bins) * _halfside[0]),3)));
    }
  }
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    penergy_temp = _vtail/ double(_npart) + 4.0 * penergy_temp / double(_npart);
  ...
  if (_measure_pressure and _measure_temp and _measure_kenergy){
    pressure_temp = _ptail + _rho*_measurement(_index_temp) + 16. * 1./_volume * pressure_temp;
    _measurement(_index_pressure) = pressure_temp;
  }
 
  // MAGNETIZATION/////////////////////////////////////////////////////////////
  if (_measure_magnet and (_sim_type > 1)) {
    double magne_temp = 0.0; //variabile per magnetizzazione
    for (int i=0; i<_npart; i++){
      magne_temp += _particle(i).getspin();
    }
    magne_temp /= double(_npart); //calcolo magnetizzazione per spin
    _measurement(_index_magnet) = magne_temp;
  }

  
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  //In realtà qua gestisco i quadrati dell'energia, cosi in averages calcolerò la media dei quadrati e solo allora potrò calcolare il calore specifico
  if (_measure_cv and (_sim_type > 1)) {
    //Calcolo prima la media dell'energia al quadrato
    double s_i, s_j;
    double e_conf = 0.0;
    for (int i=0; i<_npart; i++){
        //cout << "partic 1 spin: " <<_particle(i).getspin() << endl;
        //cout << "partic 2 spin: " <<_particle(this->pbc(i+1)).getspin() << endl;
        s_i = double(_particle(i).getspin());
        s_j = double(_particle(this->pbc(i+1)).getspin());
        e_conf += (- _J * s_i * s_j - 0.5 * _H * (s_i + s_j));
      }
      
      _measurement(_index_cv) = e_conf*e_conf;   //stampo come misura l'energia al quadrato (totale!, non per particella)
  }

  
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi and (_sim_type > 1)) {
    double susc_temp = 0.0; //variabile per suscettività
    for (int i=0; i<_npart; i++){
      //cout << "partic 1 spin: " <<_particle(i).getspin() << endl;
      susc_temp += _particle(i).getspin();
    }
  
    _measurement(_index_chi) = _beta*pow(susc_temp,2) / double(_npart); //calcolo suscettività
...
void System :: averages(int blk){
  ...
  if (_measure_tenergy){
    ...
    //Aggiungo codice per stampare valore medio ultimo blocco con relativo errore
    if (blk == _nblocks) {
      file1.open("../OUTPUT/tot_energy(T).dat", ios::app);
      file1 << setw(18) << _temp
  ...
  if (_measure_gofr){
    coutf.open("../OUTPUT/gofr.dat"); //tolgo il tag "app" cosi anzichè appendere il valore per ogni blocco, nel file finale stamperà solo l'ultima media!
    for(int i=0; i<_n_bins; i++){
    average  = _average(_index_gofr + i);
    sum_average = _global_av(_index_gofr + i);
    sum_ave2 = _global_av2(_index_gofr + i);
    coutf << setw(12) << double(i)/double(_n_bins) * _halfside[0] // è la distanza r
          // << setw(12) << average    // questo non lo stampo, non mi serve la media attuale
          << setw(12) << (sum_average) / double(blk)
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    }
    coutf.close();
  ...
  if (_measure_magnet){
    ...
    //Aggiungo codice per stampare valore medio ultimo blocco con relativo errore
    if (blk == _nblocks) {
      file2.open("../OUTPUT/magnetization(T).dat", ios::app);
      file2 << setw(18) << _temp
            << setw(18) << sum_average/double(blk)
            << setw(18) << this->error(sum_average, sum_ave2, blk) << endl;
      file2.close();
  ...
  if (_measure_cv){
    ...
    //Gestisco a parte il calcolo del calore specifico
    spec_heat = pow(_beta, 2) * (_average(_index_cv) - pow(_average(_index_tenergy) * double(_npart), 2)) / double(_npart);   // calore specifico per l'i-esimo blocco (per particella)
    _global_sh_av += spec_heat;  //incremento l'accumulatore dei valori di calore specifico, con il valore del calore specifico attuale (aka del blocco corrente)
    _global_sh_av2 += spec_heat*spec_heat; //idem ma accumulo i quadrati
    
    coutf << setw(12) << blk
          << setw(12) << spec_heat
          << setw(12) << _global_sh_av/double(blk)
          << setw(12) << this->error(_global_sh_av, _global_sh_av2, blk) << endl;
    //Aggiungo codice per stampare valore medio ultimo blocco con relativo errore
    if (blk == _nblocks) {
      file3.open("../OUTPUT/Specific_Heat(T).dat", ios::app);
      file3 << setw(18) << _temp
            << setw(18) << _global_sh_av/double(blk)
            << setw(18) << this->error(_global_sh_av, _global_sh_av2, blk) << endl;
      file3.close();
  ...
  if (_measure_chi){
    ...
    //Aggiungo codice per stampare valore medio ultimo blocco con relativo errore
    if (blk == _nblocks) {
      file4.open("../OUTPUT/Susceptibility(T).dat", ios::app);
      file4 << setw(18) << _temp
            << setw(18) << sum_average/double(blk)
            << setw(18) << this->error(sum_average, sum_ave2, blk) << endl;
      file4.close();
...
double System :: get_temperature_measurement() {
  return _measurement(_index_temp);
}

double System :: get_potential_measurement() {
  return _measurement(_index_penergy);
}
/****************************************************************
*****************************************************************
    _/    _/  _/_/_/  _/       Numerical Simulation Laboratory
